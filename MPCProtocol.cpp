...
vector<doubleNumber> MPCProtocol::obtainShares(double in_number, int in_cond)
{
    if (in_number > maxInputValue) 
    {
        maxInputValue = in_number;
    }
    ...
    // const int EXPONENT_MOD = (1 << 11);
    // const uint_fast64_t SIGNIFICAND_MOD = (1ULL << 52);
    ...
    // int min = 1023;
    // int_fast16_t max = (1 << 16);

    // int_fast16_t exp_1 = min + ((rand() % max) % EXPONENT_MOD) % (MAX_EXP_BOUND - min);
    // int_fast16_t exp_2 = min + ((rand() % max) % EXPONENT_MOD) % (MAX_EXP_BOUND - min);
    // int_fast16_t exp_3 = min + ((rand() % max) % EXPONENT_MOD) % (MAX_EXP_BOUND - min);

    
    // uint_fast64_t sig_1 = ((((uint_fast64_t)rand() << 30) | ((uint_fast64_t)rand() << 15) | (uint_fast64_t)rand()) % SIGNIFICAND_MOD) % MAX_SIG_BOUND;
    // uint_fast64_t sig_2 = ((((uint_fast64_t)rand() << 30) | ((uint_fast64_t)rand() << 15) | (uint_fast64_t)rand()) % SIGNIFICAND_MOD) % MAX_SIG_BOUND;
    // uint_fast64_t sig_3 = ((((uint_fast64_t)rand() << 30) | ((uint_fast64_t)rand() << 15) | (uint_fast64_t)rand()) % SIGNIFICAND_MOD) % MAX_SIG_BOUND;
    ...
    // double t_outputShare1 = FloatingPoint::rePackForTesting(t_share1);
    // double t_outputShare2 = FloatingPoint::rePackForTesting(t_share2);

    // bool signA3;
    // int_fast16_t expA3;
    // uint_fast64_t sigA3;

    // FloatingPoint::f64_add(t_share1.sign, t_share1.exp, t_share1.sig, t_share2.sign, t_share2.exp, t_share2.sig,
    //                       signA3, expA3, sigA3, packingProcess);

    // bool signA4;
    // int_fast16_t expA4;
    // uint_fast64_t sigA4;

    // FloatingPoint::f64_add(signA3, expA3, sigA3, sign_3, exp_3, sig_3, signA4, expA4, sigA4, packingProcess);

    // signA4 = signA4 ^ 1;

    // bool u_r_signA4;
    // int_fast16_t u_r_expA4;
    // uint_fast64_t u_r_sigA4;

    // FloatingPoint::f64_add(signA, expA, sigA, signA4, expA4, sigA4, u_r_signA4, u_r_expA4, u_r_sigA4, packingProcess);

    // FloatingPoint::f64_add(u_r_signA4, u_r_expA4, u_r_sigA4, sign_3, exp_3, sig_3,
    //                       t_share3.sign, t_share3.exp, t_share3.sig, packingProcess);

    // double t_outputShare3 = FloatingPoint::rePackForTesting(t_share3);

    // double FinalAddedShares = (FloatingPoint::rePackForTesting(t_share1) +
    //                           FloatingPoint::rePackForTesting(t_share2) +
    //                           FloatingPoint::rePackForTesting(t_share3));
